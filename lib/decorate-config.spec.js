...
describe('decorate-config', function() {
  ...
  it('sets hashRouting when build passes flag', () => {
    assert.equal(decorateConfig({}, {}, { locationType: 'hash' }).hashRouting, true,
      'flags hashRouting when build configs specify locationType hash');

    assert.equal(decorateConfig({}, {}, { locationType: 'history' }).hashRouting, null,
      'doesnt flag hashRouting when build configs specify locationType history');
  });
